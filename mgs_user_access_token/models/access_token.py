...
token_expiry_date_in = "mgs_payment_integration.access_token_token_expiry_date_in"
...
class APIAccessToken(models.Model):
    ...
    def find_or_create_token(self, user_id=None, create=False):
        ...
        access_token = self.env["mgs.api.access_token"].sudo().search([("user_id", "=", user_id)], order="id DESC", limit=1)
        if access_token:
        ...
        if not access_token and create:
            ...
            access_token = self.env["mgs.api.access_token"].sudo().create(vals)
        if not access_token:
            return None
...
class Users(models.Model):
    ...
    token_ids = fields.One2many("mgs.api.access_token", "user_id", string="Access Tokens")
