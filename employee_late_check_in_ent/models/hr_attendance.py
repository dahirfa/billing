...
class HrAttendanceLatencyAbsence(models.TransientModel):
    ...
    def check_attendance(self):
        ...
        for employee in self.employee_ids.filtered(lambda x:x.contract_id):
...
class HrAttendance(models.Model):
    ...
    def get_late_minutes(self):
        for rec in self:
            ...
            contract =  rec.sudo().employee_id.contract_id
            if contract:
                ...
                for schedule in work_schedule.sudo().attendance_ids:
                    if schedule.dayofweek == str(week_day):
                        ...
                        minutes=0
                        if contract.late_penalty_id:
                            minutes =  contract.late_penalty_id.minutes
