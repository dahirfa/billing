...
class MgsPayment(models.Model):
    ...
    def create(self, vals_list):
        ...
        records = res.filtered(lambda x: x.method_code == 'edahab_sc' and x.state not in ('cancel', 'duplicate'))
        records.filtered(lambda x: not x.partner_id).action_edahab_sc_draft()
        records.filtered(lambda x: x.partner_id).action_edahab_sc_confirm()
        return res

    def action_edahab_sc_draft(self):
        payment_obj = self.env['account.payment']
        payment_method_line_id = self.env.ref(
            'mgs_edahab_sc.payment_method_edahab_sc')
    ...
    def action_edahab_sc_confirm(self):
        payment_obj = self.env['account.payment']
        payment_method_line_id = self.env.ref('mgs_edahab_sc.payment_method_edahab_sc')
...
class PaymentProvider(models.Model):
    ...
    code = fields.Selection(selection_add=[("edahab_sc", "Edahab Short Code")], ondelete={"edahab_sc": "set default"})
      
    edahab_sc_user_id = fields.Many2one('res.users', domain =[('share','=',True)])
...
class AccountPaymentMethod(models.Model):
    ...
    def _get_payment_method_information(self):
        ...
        res["edahab_sc"] = {"mode": "unique", "domain": [("type", "=", "bank")]}
        return res
