    ...
    def _get_action_dict(self):
        ...
        for action_xml_id, (field_name, allowed_values) in restricted_actions.items():
            if action.get("id") == self.env.ref(action_xml_id).id and journal_restrict_group:
                existing_domain = action.get("domain", [])  # Get existing domain
                # if not isinstance(existing_domain, list):
                #     existing_domain = [] if existing_domain is None else [existing_domain]
                existing_domain = [existing_domain]
                new_condition = [(field_name, 'in', allowed_values)]
                action['domain'] = new_condition  # Append restriction
