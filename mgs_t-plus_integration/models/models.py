...
class MgsPayment(models.Model):
    ...
    def create(self, vals_list):
        ...
        tplus_recs = res.filtered(lambda x: x.method_code == 't_plus' and x.state not in ('cancel', 'duplicate'))
        tplus_recs.filtered(lambda x: not x.partner_id).action_t_plus_draft()
        tplus_recs.filtered(lambda x: x.partner_id).action_t_plus_confirm()
        return res

    def action_t_plus_draft(self):
        payment_obj = self.env['account.payment']
        payment_method_line_id = self.env.ref(
            'mgs_t-plus_integration.payment_method_t_plus')
    ...
    def action_t_plus_confirm(self):
        payment_obj = self.env['account.payment']
        payment_method_line_id = self.env.ref(
            'mgs_t-plus_integration.payment_method_t_plus')
        if not payment_method_line_id:
...
class PaymentProvider(models.Model):
    ...
    code = fields.Selection(
        selection_add=[('t_plus', "T-Plus")], ondelete={'t_plus': 'set default'})
    t_plus_user_id = fields.Many2one(
...
class AccountPaymentMethod(models.Model):
    ...
    def _get_payment_method_information(self):
        ...
        res['t_plus'] = {'mode': 'unique', 'domain': [('type', '=', 'bank')]}
        return res
