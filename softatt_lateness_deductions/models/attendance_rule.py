...
class MsgAttendanceRule(models.Model):
    ...
    absence_deduction       = fields.Boolean(default=False)
    absence_deduction_type  = fields.Selection([('wage', 'Daily Wage'), ('fixed_amount','Fixed Amount')])
    absence_fixed_amount    = fields.Monetary(string='Absence Deduction Amount')
    multiply_with           = fields.Float()
    daily_wage_base         = fields.Selection([('30', 'Average(30 Days)'),
                                               ('month_based','Month Based')])
    time_minutes            = fields.Integer(string='Time(Minutes)')
    ...
    def _compute_penalty(self, employee, date, minutes):
