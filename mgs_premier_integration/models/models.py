...
class MgsPayment(models.Model):
    ...
    def create(self, vals_list):
        ...
        records = res.filtered(lambda x: x.method_code == 'premier' and x.state not in ('cancel', 'duplicate'))
        records.filtered(lambda x: not x.partner_id).action_premier_draft()
        records.filtered(lambda x: x.partner_id).action_premier_confirm()
        return res

    def action_premier_draft(self):
        payment_obj = self.env['account.payment']
        payment_method_line_id = self.env.ref(
            'mgs_premier_integration.payment_method_premier')
    ...
    def action_premier_confirm(self):
        payment_obj = self.env['account.payment']
        payment_method_line_id = self.env.ref('mgs_premier_integration.payment_method_premier')
...
class PaymentProvider(models.Model):
    ...
    code = fields.Selection(
        selection_add=[('premier', "Premier")], ondelete={'premier': 'set default'})
    premier_user_id = fields.Many2one(
...
class AccountPaymentMethod(models.Model):
    ...
    def _get_payment_method_information(self):
        ...
        res['premier'] = {'mode': 'unique', 'domain': [('type', '=', 'bank')]}
        return res
