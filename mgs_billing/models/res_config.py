...
class ResCompany(models.Model):
    ...
    mgs_billing_global_average = fields.Boolean(
        default=False, string='Use default average')
    ...
    mgs_plan_categ_id = fields.Many2one(
        'product.category', string='Plan Category')
...
class ResConfigSettings(models.TransientModel):
    ...
    mgs_billing_global_average = fields.Boolean(
        default=False, string='Use default average', related="company_id.mgs_billing_global_average", readonly=False)
    mgs_billing_if_its_less_than = fields.Float(
        related="company_id.mgs_billing_if_its_less_than", readonly=False, string="If it's less than")
    ...
    mgs_extra_service_ids = fields.Many2many(
        'product.product', string='Extra Services/Charges', related="company_id.mgs_extra_service_ids", readonly=False, domain=[(
            'type', '=', 'service')])

    mgs_billing_global_seq = fields.Boolean(
        string='Use a global sequence', related="company_id.mgs_billing_global_seq", readonly=False)

    mgs_billing_global_seq_id = fields.Many2one(
        'ir.sequence', string='Sequence', related="company_id.mgs_billing_global_seq_id", readonly=False)

    billing_period_end = fields.Integer(
        string='Billing Period (Start)', related="company_id.billing_period_end", readonly=False)
    billing_period_start = fields.Integer(
        string='Billing Period (Endd)', related="company_id.billing_period_start", readonly=False)
    allowed_reg_date = fields.Integer(
        string='Allowed Registeration Date', related="company_id.allowed_reg_date", readonly=False)

    mgs_plan_categ_id = fields.Many2one(
        'product.category', string='Plan Category', related="company_id.mgs_plan_categ_id", readonly=False)
