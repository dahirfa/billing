...
class MgsTPlusIntegration(http.Controller):
    ...
    def yeel_authenticate(self, key, db):
        payment_method = request.env.ref(
            'mgs_daryeel_integration.payment_provider_yeel')
        user_id = payment_method.sudo().yeel_user_id
        try:
    ...
    @http.route('/api/values/yeel/getinfo/<int:meter>', methods=["GET", "POST"], type='http',  auth="public", csrf=False)
    def bill_info(self, meter, **kw):
        db = request._cr.dbname
        headers = request.httprequest.headers
        authenticate = self.yeel_authenticate(headers.get('key'), db)
    ...
    # @validate_token
    @http.route('/api/values/yeel/paymentinfo', methods=["GET", "POST"], type='http',  auth="none", csrf=False)
