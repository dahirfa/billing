...
class MgsPayment(models.Model):
    ...
    def create(self, vals_list):
        ...
        yeel_recs = res.filtered(lambda x: x.method_code == 'yeel' and x.state not in ('cancel', 'duplicate'))
        yeel_recs.filtered(lambda x: not x.partner_id).action_yeel_draft()
        yeel_recs.filtered(lambda x: x.partner_id).action_yeel_confirm()
        return res


    def action_yeel_draft(self):
        payment_obj = self.env['account.payment']
        payment_method_line_id = self.env.ref(
            'mgs_daryeel_integration.payment_method_yeel')
...
class PaymentProvider(models.Model):
    ...
    code = fields.Selection(
        selection_add=[('yeel', "Yeel")], ondelete={'yeel': 'set default'})
    yeel_user_id = fields.Many2one(
...
class AccountPaymentMethod(models.Model):
    ...
    def _get_payment_method_information(self):
        ...
        res['yeel'] = {'mode': 'unique', 'domain': [('type', '=', 'bank')]}
        return res
