    ...
    def _fill_bank_cash_dashboard_data(self, dashboard_data):
        ...
        for journal in self.filtered(lambda j: j.type in ('bank', 'cash')):
            if journal.id in dashboard_data:
                currency = journal.currency_id if journal.currency_id else self.env['res.currency'].browse(journal.company_id.sudo().currency_id.id)
                
                # Check if the journal's currency matches the company currency
                if not journal.currency_id or journal.currency_id.id == journal.company_id.currency_id.id:
                    # Use current_balance in company currency
                    acc_balance = self._mgs_sum_open_balance(journal.default_account_id.id)
                else:
                    # Use function to calculate balance in foreign currency
                    acc_balance = self._mgs_sum_open_balance_fc(journal.default_account_id.id)
