...
class MgsPartnerLedger(models.Model):
    _name = 'mgs.account.partner.ledger.report.handler'

    def _get_options_initial_balance(self, options):
        new_date_to = fields.Date.from_string(
            options['date']['date_from']) - timedelta(days=1)
    ...
    def _get_initial_balance_values(self, partner_id, options):
        queries = []
        params = []
        report = self.env.ref('account_reports.partner_ledger_report')
        ct_query = self.env['res.currency']._get_query_currency_table(options)
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            new_options = self._get_options_initial_balance(
                column_group_options)
            tables, where_clause, where_params = report._query_get(
                new_options, 'normal', domain=[('partner_id', '=', partner_id)])
            params += where_params
            queries.append(f"""
                SELECT
    ...
    def _get_mgs_aml_values(self, options, partner_ids, offset=0, limit=None):
        ...
        if partner_ids_wo_none:
            ...
            directly_linked_aml_partner_params.append(
                tuple(partner_ids_wo_none))
            indirectly_linked_aml_partner_clause = 'aml_with_partner.partner_id IN %s'
            indirectly_linked_aml_partner_params.append(
                tuple(partner_ids_wo_none))
        directly_linked_aml_partner_clause = '(' + ' OR '.join(
            directly_linked_aml_partner_clauses) + ')'
