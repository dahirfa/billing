...
class AccountPayment(models.Model):
    ...
    bulk_payment_id = fields.Many2one(
        'mgs_necsom_addons.bulk_payment', string='Bulk Payment')
    ...
    def action_open_mgs_payment(self):
        ...
        if self.bulk_payment_id:
            action = self.env.ref(
                'mgs_necsom_addons.action_bulk_payment').sudo().read()[0]
            action['views'] = [
                (self.env.ref('mgs_necsom_addons.view_bulk_payment_form').id, 'form')]
...
class BulkPayment(models.Model):
    _name = 'mgs_necsom_addons.bulk_payment'
    ...
    payment_lines = fields.One2many(
        'mgs_necsom_addons.bulk_payment_line', 'bulk_payment_id', string='Payment Lines')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['name'] = vals['name'] = self.env['ir.sequence'].next_by_code(
                'mgs_necsom_addons.bulk_payment') or '/'
...
class BulkPaymentLine(models.Model):
    _name = 'mgs_necsom_addons.bulk_payment_line'
